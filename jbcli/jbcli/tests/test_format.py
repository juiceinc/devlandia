...
class TestFormat:
    ...
    def test_echo_highlight(self, secho_mock):
        ...
        assert secho_mock.mock_calls == [call("COOKIES!", fg="yellow", bold=True)]

    @patch("jbcli.utils.format.secho")
    def test_echo_success(self, secho_mock):
        echo_success("COOKIES!")
        assert secho_mock.mock_calls == [call("COOKIES!", fg="green", bold=True)]

    @patch("jbcli.utils.format.secho")
    def test_echo_warning(self, secho_mock):
        echo_warning("COOKIES!")
        assert secho_mock.mock_calls == [call("COOKIES!", fg="red", bold=True)]
